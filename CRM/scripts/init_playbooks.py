...
PLAY={'demo':{'pitch':'value','pain':['p1']}}
def run():
    conn=psycopg2.connect(**DB); cur=conn.cursor()
    cur.execute("create table if not exists sales_playbooks(id serial primary key, tag varchar(200) unique, topic jsonb)")
    for k,v in PLAY.items():
        cur.execute("insert into sales_playbooks(tag,topic) values(%s,%s) on conflict(tag) do update set topic=excluded.topic",(k,json.dumps(v)))
