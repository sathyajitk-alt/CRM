...
def create_contact(account_id, name, designation, email, phone, linkedin):
    if not email:
        return None
    ...
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO contacts(account_id, name, designation, email, phone, linkedin)
        VALUES (NOW(),%s,%s,%s,%s,%s,%s)
        ON CONFLICT(email) DO NOTHING
        RETURNING id""", (
            account_id, name, designation, email, phone, linkedin
        )
    )
    row = cur.fetchone()
    conn.commit()
