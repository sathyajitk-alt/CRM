...
def clean_date(value):
    ...
    # ACCESS CONTROL: enforce user_can_view_account / user_can_edit_account here
def create_opportunity(parent_account_id, row):
    lob=row.get('LOB')
    if not lob: return None
    conn=get_conn(); cur=conn.cursor()
    cur.execute('INSERT INTO opportunities (created_at,account_id,lob,premium,dor,insurer) VALUES (NOW(),%s,%s,%s,%s,%s) RETURNING id',
        (parent_account_id,row.get('LOB'),row.get('Premium'),clean_date(row.get('DOR')),row.get('Insurer')))
    oid=cur.fetchone()[0]
