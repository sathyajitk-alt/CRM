...
def create_account(name, domain=None, industry=None, revenue=None, size=None, location=None):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO accounts(name, domain, industry, revenue, size, location)
             VALUES (NOW(),%s, %s, %s, %s, %s, %s) RETURNING id""",
        (name, domain, industry, revenue, size, location)
    )
